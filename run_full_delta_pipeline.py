...
def merge_tx_counts():
    ...
    try:
        if not os.path.exists("wallet_tx_counts_delta.csv"):
            print("⚠️ wallet_tx_counts_delta.csv not found, skipping merge.")
            return

        df_wallets = pd.read_csv("delta_wallets.csv")
        df_tx = pd.read_csv("wallet_tx_counts_delta.csv")
        ...
        merged.to_csv("delta_wallets.csv", index=False)
